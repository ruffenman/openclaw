...
- `speechLocale` sets the BCP 47 locale id used by Android, iOS, and macOS Talk speech recognition and by the iOS system-voice fallback. Android also uses its language component to guide realtime input transcription. macOS forwards the two-letter language component from an explicit locale to realtime input transcription; unset or `auto` keeps automatic language detection. Leave unset to use the device default for native recognition.
